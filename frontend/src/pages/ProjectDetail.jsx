...
import {
  ...
  Trash2, Search, X, AlertTriangle, UserPlus, UserMinus,
  ChevronDown,
} from 'lucide-react'
...
function TaskCard({ task, onEdit, onDelete }) {
  ...
  return (
    <div
      onClick={() => onEdit(task)}
      className={clsx('rounded-xl p-3.5 group transition-all duration-150 cursor-pointer animate-fade-in')}
      style={{
        background: 'var(--surface-3)',
        border: overdue ? '1px solid rgba(239,68,68,0.4)' : '1px solid var(--border)',
      }}
      onMouseEnter={(e) => { if (!overdue) e.currentTarget.style.borderColor = 'var(--border-strong)' }}
    ...
    >
      ...
      <div className="flex items-center justify-between">
        ...
        {task.due_date && (
          <div className={clsx('flex items-center gap-1 text-xs', overdue ? 'text-red-400 font-medium' : '')}
      ...
      {task.assignee && (
        <div className="mt-2 pt-2 flex items-center gap-1.5" style={{ borderTop: '1px solid var(--border)' }}>
          ...
          <span className="text-xs truncate" style={{ color: 'var(--text-muted)' }}>{task.assignee.name}</span>
        </div>
      )}
    </div>
  )
...
// ── AddMemberPanel ────────────────────────────────────────────────────────────
function AddMemberPanel({ projectId, currentMembers, onAdded }) {
  const [open, setOpen] = useState(false)
  const [search, setSearch] = useState('')
  const [role, setRole] = useState('member')
  const [adding, setAdding] = useState(null) // userId sendo adicionado
  ...
  const handleAdd = async (user) => {
    ...
    setError('')
    try {
      await projectsApi.addMember(projectId, { user_id: user.id, role_in_project: role })
      onAdded()
    } catch (err) {
  ...
  return (
    <div className="relative">
      ...
      {open && (
        <div
          className="absolute top-full left-0 mt-2 w-80 rounded-xl shadow-2xl z-30 animate-slide-up"
        ...
        >
          <div className="p-3 flex flex-col gap-2" style={{ borderBottom: '1px solid var(--border)' }}>
            ...
            >
              <option value="member">Membro — pode criar e editar tarefas</option>
              <option value="viewer">Visitante — só pode visualizar</option>
              <option value="owner">Co-dono — acesso total</option>
            </select>
          </div>
          ...
          <div className="max-h-56 overflow-y-auto">
            {isLoading ? (
              ...
              available.map((u) => (
                ...
                >
                  {/* Avatar */}
                  <div className="w-8 h-8 rounded-lg bg-brand/20 border border-brand/30 flex items-center justify-center font-semibold text-brand text-sm shrink-0">
                  ...
                  >
                    {adding === u.id ? <Spinner size={12} /> : <><Plus size={12} /> Adicionar</>}
                  </button>
                </div>
              ))
            )}
          ...
          <div className="p-2 flex justify-end" style={{ borderTop: '1px solid var(--border)' }}>
            <button
...
export default function ProjectDetail() {
  ...
  const updateTask = useMutation({
    ...
    onSuccess: (_, variables) => {
      ...
      closeTaskModal()
      if (variables.data.status === 'done') fireConfetti()
    },
    onError: (err) => setTaskError(getErrorMessage(err)),
  })
  ...
  const handleTaskSubmit = (e) => {
    ...
    if (taskModal.task) updateTask.mutate({ taskId: taskModal.task.id, data: payload })
  ...
  const tasksByStatus = (status) => filterTasks(tasks).filter((t) => t.status === status)
  const overdueCount = (tasks || []).filter(isOverdue).length

  if (loadingProject) {
    return <div className="p-8 flex items-center justify-center h-64"><Spinner size={24} className="text-brand" /></div>
  }
  if (!project) {
    return (
      <div className="p-8">
  ...
  return (
    <div className="flex flex-col min-h-screen">
      {/* ── Top bar ─────────────────────────────────────────────────── */}
      <div className="sticky top-0 z-10 backdrop-blur-md px-8 py-4"
        ...
        <div className="flex items-center gap-0 mt-3 -mb-4">
          {TABS.map((tab) => (
            <button key={tab.key} onClick={() => setActiveTab(tab.key)}
              className={clsx('px-4 py-2 text-sm font-medium border-b-2 transition-all',
          ...
          <>
            <div className="flex items-center gap-3 mb-5">
              <div className="relative max-w-xs w-full">
                ...
                <input className="input pl-9 pr-8" placeholder="Buscar tarefas..."
                ...
                return (
                  <div key={key} className="flex flex-col gap-3">
                    <div className="flex items-center justify-between">
                      ...
                      <button onClick={() => openCreateTask(key)}
                        className="btn-icon w-6 h-6 hover:text-brand" style={{ color: 'var(--text-muted)' }}>
                        <Plus size={13} />
                      </button>
                    </div>
                    <div className="flex flex-col gap-2 min-h-[8rem]">
                      {loadingTasks ? (
                        ...
                        <TaskCard key={t.id} task={t} onEdit={openEditTask}
          ...
          <div className="max-w-lg flex flex-col gap-4">
            ...
            {isOwner && (
              <AddMemberPanel
                ...
                onAdded={() => qc.invalidateQueries({ queryKey: ['project', id] })}
              />
            )}
            ...
            <div className="flex flex-col gap-2">
              {project.members?.length === 0 ? (
                <Empty icon={UserPlus} title="Nenhum membro ainda"
                  description="Adicione membros para colaborar no projeto." />
              ) : (
                  ...
                  <div key={log.id} className="flex items-start gap-3 py-3"
                    ...
                    <div>
                      <p className="text-sm" style={{ color: 'var(--text-secondary)' }}>
                        <span className="capitalize font-medium" style={{ color: 'var(--text-primary)' }}>{log.action}</span>
                        {' '}{log.entity_type}
      ...
      {/* ── Task Modal ──────────────────────────────────────────────── */}
      <Modal open={taskModal.open} onClose={closeTaskModal}
        title={taskModal.task ? 'Editar Tarefa' : 'Nova Tarefa'} size="lg">
        <form onSubmit={handleTaskSubmit} className="flex flex-col gap-4">
          <Alert type="error" message={taskError} />
          <Input label="Título *" placeholder="Descreva a tarefa" value={taskForm.title}
            onChange={(e) => setTaskForm((f) => ({ ...f, title: e.target.value }))} autoFocus />
          <Textarea label="Descrição" placeholder="Detalhes opcionais..." value={taskForm.description}
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <Select label="Prioridade" value={taskForm.priority}
          ...
          <div className="grid grid-cols-2 gap-3">
            <Input label="Data limite" type="date" value={taskForm.due_date}
              onChange={(e) => setTaskForm((f) => ({ ...f, due_date: e.target.value }))} />
            <Select label="Responsável" value={taskForm.assignee_id}
